...
// BEGIN CODE
import yellowSquare from "../assets/yellowSquare.jpg"
import tstImg from "../assets/tstImgLink.jpg"
  ...
  async update(oldData, forceLocalRefresh) {
    ...
    try {
      if ((forceLocalRefresh || srcChanged) && !this.showLoaderTimeout) {
        this.showLoaderTimeout = setTimeout(this.showLoader, 5000);
      }
      ...
      if (
        ...
        // BEGIN CODE 
        // Definition of object to be used next, in order to have choice of png
        let srcTmp =  yellowSquare;

        //console.log("src => " + src);
        //console.log("data.src => " + this.data.src);

        if(src.includes("romamile")) {
            srcTmp =  tstImg;
        }
        ...
        this.el.setAttribute(
          "media-image",
          Object.assign({}, this.data.mediaOptions, objTmp)
        );
        if (this.el.components["position-at-border__freeze"]) {
          this.el.setAttribute("position-at-border__freeze", { isFlat: true });
        }
