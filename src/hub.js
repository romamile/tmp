...
// == romamile
var baseNbrOfStream = -1;

let trigSys = {}
trigSys.timeout = null;
trigSys.OK = false;
trigSys.listObj = {};
trigSys.listTrig = {};
trigSys.listAnim = [];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const defaultRoomId = configs.feature("default_room_id");
	...
	// 0) Definitions
    let myUser = {idAvatar:0, firstName:"Hello", lastName:"World", role:""};

	//if(window.location.href.includes("admin"))
	//	myRole = "admin"

	// 1) Get info from the server about user
    if (!qs.has("hashUser")) { 	// No hash in query => not in the user list
		console.log("Sorry, you're not on the user list");
        //store.initProfileFromJson(myUser);

	} else {	// In the user list
		console.log("Congrats, you're on the user list");
	
            // 1) get the hash code
      	const hashUser = qs.get("hashUser");
		console.log("you're an admin, lucky you");

			// 1.5) get the correct avatarId
        store.state.profile.avatarId = "https://phantom-galaxy.com:52000/avatar?hashUser="+hashUser;

		    // 2) Make a request on the DB to get ID, display name, idAvatar (and probably status of burger panel...)
		let linkToDB = "https://phantom-galaxy.com:52000/userStatus?hashUser="+hashUser;
		var xhr = new XMLHttpRequest();
		xhr.open("GET", linkToDB, false);
			// BAD BAD BAD => the request is "synch" (should be asynchrone),
				//only because os usage of idAvatar. Check how can be done asynch

		xhr.onload = function (e) {
		  if (xhr.readyState === 4) {
			if (xhr.status === 200) {

			  myUser = JSON.parse(xhr.responseText);
			  console.log(myUser);
	...
	// 2) Send to server info about how many people are there
	var timerID_updateRoomStatus = setInterval( () => {
		...
		let linkTo = "https://phantom-galaxy.com:52000/userPresent?hashUser="+hashUser+"&roomSublink="+window.location.pathname;
		var xhr = new XMLHttpRequest();
	...
	if      (window.location.href.includes("brand") ) { window.myRoom = "brand"; }
	else if (window.location.href.includes("design"  ) ) { window.myRoom = "design"; }
	else if (window.location.href.includes("fragrance"  ) ) { window.myRoom = "fragrance"; }
	else if (window.location.href.includes("ceremony"  ) ) { window.myRoom = "ceremony"; }
	else if (window.location.href.includes("communication"  ) ) { window.myRoom = "communication"; }
	else if (window.location.href.includes("farewell"  ) ) { window.myRoom = "farewell"; }


	// Regularly check if you have camera open If it's the case, then put it on you
	if(myUser.role != "user") {
 
		window.timerIDcam = setInterval(function() {

			//var indexStream = baseNbrOfStream; // +1 - 1 = + 0
			var indexStream = 0;

			// 1) Check if there are streams, and if they are mine
			var listVidElt = AFRAME.scenes[0].querySelectorAll("[media-video]");
			var myVids = [];
			for (let e of listVidElt){
				if(typeof e.attributes["body-helper"] !==  "undefined") {
					if(e.attributes["body-helper"].nodeValue === "") {
						myVids.push(e)
					}
				}
			}

			if(myVids.length >= 0 && baseNbrOfStream == -1) {
				baseNbrOfStream = myVids.length;	
				//console.log("base nbr of stream = " + baseNbrOfStream);
				return;
			}

			if(myVids.length < indexStream +1) {
				//console.log("No personal stream present. NbrStream: "+myVids.length);
				return;
			}

//			if(myVids.length == 0) {
//				console.log("No personal stream present");
//				return;
//			}
//
//
//			if(myVids.length > 1) {
//				console.log("More than one personal stream present, weird...");
//			}

			// 2) Check if student or teacher
			// 3) Move stream to correct location
			//console.log("bNbr: "+baseNbrOfStream+", idStream: "+indexStream+"/"+myVids.length);
			var camEl = myVids[indexStream];

			//console.log("stream found! myrole: " + myUser.role);

			var selfEl = AFRAME.scenes[0].querySelector("#avatar-rig")
			var povCam = selfEl.querySelector("#avatar-pov-node")

			NAF.utils.getNetworkedEntity(camEl).then(networkedEl => { 
			  
			  // Get ownership of element
			  const mine = NAF.utils.isMine(networkedEl)
			  if (!mine) var owned = NAF.utils.takeOwnership(networkedEl)

			  if (myUser.role === "admin" || myUser.role === "mod") {

				  let sPx = 0, sPy = 0, sPz = 0, sScale = 1
				  let sRx = -Math.PI, sRy = 0, sRz = -Math.PI;
				
				  var selfPos = AFRAME.scenes[0].querySelector("#avatar-rig").object3D.position;
				  var refCam = {}

				  let minDist = 9999999;
				  for(const ref of trigSys.dbTrig.campos) {
					if(selfPos.distanceTo( ref.pos ) < minDist) {
					  minDist = selfPos.distanceTo( ref.pos );
					  refCam = ref;
					}
				  }
				  ...
				  // Update of values
				  if(minDist < 10000) {
					  networkedEl.object3D.scale.setScalar( sScale );

					  networkedEl.object3D.rotation.x = refCam.rot.x 
					  networkedEl.object3D.rotation.y = refCam.rot.y
					  networkedEl.object3D.rotation.z = refCam.rot.z

					  networkedEl.object3D.position.x = refCam.pos.x
					  networkedEl.object3D.position.y = refCam.pos.y
					  networkedEl.object3D.position.z = refCam.pos.z
    ...
    // Animation system
	 ...
	 trigSys.playTrig = function(_trig) {
	 	console.log("PLAYING TRIG: "+_trig.name);
		_trig.played = true;
		switch(_trig.action.type) {
		case "spawn":

			let el = document.createElement("a-entity")
			AFRAME.scenes[0].appendChild(el)
			trigSys.listObj[_trig.name] = el;

			console.log(_trig.target.src)
			el.setAttribute("media-loader", { src: _trig.target.src, resolve: true })

			el.object3D.position.set(_trig.action.pos.x, _trig.action.pos.y, _trig.action.pos.z); 
			el.object3D.rotation.set(_trig.action.rot.x, _trig.action.rot.y, _trig.action.rot.z); 
			el.object3D.scale.set(_trig.action.scale.x, _trig.action.scale.y, _trig.action.scale.z); 

			switch(_trig.target.type) {
			case "image": 
				break;
			case "audio": 
			case "video": 
				let intEnded = setInterval(()=>{
					...
					if(typeof el.components["media-video"].video === "undefined") {
						// SHOULDN'T BE THERE
						// End ?
						//el.object3D.position.y = 9999999;
						//_trig.ended = true;	
						return;
					}
					...
					if(el.components["media-video"].video.ended) {
						...
						console.log("ENDED TRIG: "+_trig.name);
						el.object3D.position.y = 9999999;
						trigSys.listObj[_trig.name] = null;
						_trig.ended = true;	

						// clear interval
						clearInterval(intEnded);
					}
				}, 100);
				// Play / Pause
				//el.components["media-video"].tryUpdateVideoPlaybackState(false, 0);
				
				// Has Ended or not
				//el.components["media-video"].video.ended
				break;
			case "glb": 
				break;
			}
		...
		case "move":
			let myObj = trigSys.listObj[_trig.target.src].object3D;
            trigSys.listObj[_trig.target.src].object3D.matrixAutoUpdate = true; 


			console.log("I'm trying to move!!!!");

			//anim.reverse()
		    let animPos = AFRAME.ANIME.default.timeline({
					targets: myObj.position, autoplay: true, easing: 'easeInOutSine', duration: _trig.action.duration})
			animPos.add(_trig.action.pos);
			animPos.play();

			/*
		    let animPos = AFRAME.ANIME.default.timeline({
					targets: myObj.position, x: _trig.action.pos.x, y: _trig.action.pos.y, z:_trig.action.pos.z,
					autoplay: true, easing: 'easeInOutSine', duration: _trig.action.duration,
...
*/
		    let animRot = AFRAME.ANIME.default.timeline({
					targets: myObj.rotation, autoplay: true, easing: 'easeInOutSine', duration: _trig.action.duration})
			animRot.add(_trig.action.rot);
			animRot.play();

		    let animScale = AFRAME.ANIME.default.timeline({
					targets: myObj.scale, autoplay: true, easing: 'easeInOutSine', duration: _trig.action.duration})
			animScale.add(_trig.action.scale);
			animScale.play();

			console.log(myObj);
			console.log(animPos);

			trigSys.listAnim.push(animPos);
			trigSys.listAnim.push(animRot);
			trigSys.listAnim.push(animScale);
			break;

		case "delete":
			trigSys.listObj[_trig.target.src].object3D.position.y = 9999999;
            trigSys.listObj[_trig.target.src].object3D.matrixAutoUpdate = true; 
			trigSys.listObj[_trig.target.src] = null;
			_trig.ended = true;	
			break;

		case "ui":
			console.log(_trig.target.src);
    		document.getElementById(_trig.target.src).style.display = "block";
			break;
	 	}
	}

	
	// Selection of the right file (should be in DB....)
	let dbLink="void";
	     if (window.location.href.includes("B5A6AS3") ) { dbLink = "https://romamile.github.io/sp_repo/db_w1.json"; }
	else if (window.location.href.includes("vDdRTxm") ) { dbLink = "https://romamile.github.io/sp_repo/db_w2.json"; }
	else if (window.location.href.includes("mTkUJEm") ) { dbLink = "https://romamile.github.io/sp_repo/db_w3.json"; }
	else if (window.location.href.includes("iUxssE9") ) { dbLink = "https://romamile.github.io/sp_repo/db_w4.json"; }
	else if (window.location.href.includes("EPvYtJp") ) { dbLink = "https://romamile.github.io/sp_repo/db_w5.json"; }
	else if (window.location.href.includes("zpLyFex") ) { dbLink = "https://romamile.github.io/sp_repo/db_w6.json"; }

     if(dbLink!="void")
	 fetch(proxiedUrlFor(dbLink))
	  .then(function (response) {
		return response.json();
	  })
	  .then(function (data) {
		trigSys.dbTrig = data;
		trigSys.dbTrig.listTrig.map( d => {
			d.triggered = false;
			d.played = false;
			d.ended = false;
		});

		trigSys.regCheck = function() {
			if(!trigSys.OK) {
				console.log("nooooooo");
  				if(scene.is("entered") && trigSys.timeout == null) {
					console.log("triggering a time out")
					trigSys.timeout = setTimeout(() => trigSys.OK = true, 1000); 
				}
				return;
		    }	
			...
			trigSys.dbTrig.listTrig.map( d => {
				...
				case "start":
					d.triggered = true;
					setTimeout(()=>trigSys.playTrig(d), d.trig.time);
				    break;

				case "proximity":
				  	let selfPos = AFRAME.scenes[0].querySelector("#avatar-rig").object3D.position;
					if(selfPos.distanceTo( new THREE.Vector3(d.trig.pos.x, d.trig.pos.y, d.trig.pos.z) ) < d.trig.radius) {
						d.triggered = true;
						setTimeout(()=>trigSys.playTrig(d), d.trig.time);
					}
					break;
				...
				case "endtrig":
					trigSys.dbTrig.listTrig.map( dRef => {
						if(dRef.name == d.trig.name && dRef.ended) {
							d.triggered = true;
							setTimeout(()=>trigSys.playTrig(d), d.trig.time);
						}
					});
		...
		trigSys.intRegCheck = setInterval(trigSys.regCheck, 500);
